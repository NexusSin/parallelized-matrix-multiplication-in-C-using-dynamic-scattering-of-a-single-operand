...
static double const epsilon = 1e-8;
...
static bool test_DGEMV(int const m, int const n, BLAS_DGEMV dgemv, double const epsilon, unsigned int const seed, double* const duration)
{
    ...
    for (int i = 0; i < m; i++) {
        ...
        err += diff*diff;
        norm += y_test[i]*y_test[i];
    }
    err = sqrt(err);
    norm = sqrt(norm);
    
    bool result_is_correct = (norm > 1e-10) ? (err / norm < epsilon) : (err < epsilon);
    ...
    double const beta, double* const y, int const incy)
{
    ...
    for (int j = 0; j < n; j++) {
        double const alpha_xj = alpha * x[j * incx];
        for (int i = 0; i < m; i++) {
            y[i * incy] += alpha_xj * A[A_IDX(i, j, ldA)];
        }
    }
}
